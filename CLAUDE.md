...
The app is live on GitHub Pages at `https://alexanderchia.github.io/comtrade-analyzer`. `main` is the deployed branch — only merge `dev` into `main` when the user explicitly asks.
...
`demo-fault.cff` in the repo root is a synthetic 3-phase A-phase fault record in CFF format, generated by a Python script. Use it to test CFF loading. It matches the data produced by `buildDemoRecord()` in `app.js`.

## Environment notes

- `node` is not available on this machine — use `python3` for any local scripting tasks.
